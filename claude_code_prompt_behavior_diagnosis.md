# Claude Code 작업 지시: 방향성 테스트 FAIL 58% 진단

## 배경

`behavioral_tests_national.py`의 테스트 2 (방향성 테스트)가 **FAIL 57.6%** 로 나왔고
(통과 기준 90%), 테스트 5 (region_code 불변성)도 FAIL 했음. 트랜스포머 진입 전에
원인 진단을 하기로 결정. 어제 합의한 옵션 B (진단 1시간) + perturbation 크기 분석 추가.

진단 결과로 트랜스포머 설계 방향이 정해짐 (lag attention vs 일사량 attention 강조 여부 등).

## 작업

1. 아래 스크립트를 `src/diagnostics/diagnose_directional_test.py`로 저장
2. `python -m src.diagnostics.diagnose_directional_test` 실행 (또는 프로젝트 컨벤션대로)
3. 실행 결과 보고

## 환경 가정 (기존 behavioral_tests_national.py와 동일)

- 입력 데이터: `data/processed/national_test_features.csv`
- 모델: `models/national_xgboost_model.json`
- 일사량 컬럼명: `일사량` (한글)
- daytime 필터: `is_daytime == 1`
- 피처 추출 방식: `[c for c in test.columns if c not in ["power_mwh", "timestamp", "region"]]`

만약 위 경로가 프로젝트에서 다르면 (예: 디렉토리 구조 변경) **그대로 두지 말고 알려줄 것**.
임의로 경로 바꿔서 실행하면 결과가 달라질 수 있음.

## 진단 5가지 (한 번에 실행됨)

1. **지역별 방향성 비율** — 17개 지역 각각, 지역당 100 샘플 (기존 10에서 확대)
2. **시간대별 방향성 비율** — 아침/오전/한낮/오후/저녁 5개 구간 (각 500 샘플)
3. **샘플링 안정성** — random_state 0~9로 10번 재실행
4. **Feature importance** — gain/weight/cover 3개 기준, 일사량/region_code/lag 순위 강조
5. **Perturbation 크기 sensitivity** — +0.1/+0.5/+1.0/+2.0/+3.0/+5.0
   - 5-A: 일사량만 변경 (기존 테스트 방식)
   - 5-B: 일사량 + irrad_x_solar 동시 변경 (파생 피처 동기화)
   - **5-A와 5-B의 차이가 클 경우 → 기존 테스트가 파생 피처 동기화를 안 해서 인위적으로
     비율이 낮았다는 증거. 트랜스포머에서도 같은 문제 반복 가능.**

## 출력물 (5개)

- `outputs/diagnosis_directional_test.json` — 모든 수치
- `outputs/diagnosis_directional_test.md` — 사람이 읽는 요약
- `outputs/diagnosis_by_region.png` — 진단 1 막대그래프
- `outputs/diagnosis_by_hour.png` — 진단 2 막대그래프
- `outputs/diagnosis_perturbation_size.png` — 진단 5 곡선

## 보고 양식

실행 완료 후, **터미널 stdout 전체를 그대로** 채팅창에 붙여줄 것 (잘라내지 말 것).
추가로 다음 4가지 확인:

1. 진단 1 결과 (지역별) — 가장 나쁜 3개 지역, 가장 좋은 3개 지역, 통과(90%↑) 지역 수
2. 진단 4 결과 (importance) — 일사량/irrad_x_solar/region_code 순위와 gain 수치
3. 진단 5-A vs 5-B 차이 — perturbation +2.0에서 두 방식의 비율 차이
4. 에러 또는 경고 — 경로 문제, 결측치, 타입 변환 경고 등 무엇이든

## 실행 후 다음 단계

진단 결과를 채팅창에 붙이면, 우리(상위 LLM)가 결과를 해석해서
다음 셋 중 하나를 결정:
- **A. XGBoost 한계 확정** → 트랜스포머 설계에 반영
- **B. 테스트 설계 결함** → 새 테스트 기준 정의 후 재측정
- **C. 부분 문제** → 특정 지역/시간대만 보정

---

## 스크립트 (그대로 저장)

아래 스크립트를 `src/diagnostics/diagnose_directional_test.py`로 저장하고 실행해줘.
디렉토리(`src/diagnostics/`)가 없으면 만들 것. `__init__.py` 필요하면 빈 파일로 추가.
...
- **모델 재학습 금지** — 기존 `models/national_xgboost_model.json` 그대로 사용
- **테스트 데이터 재생성 금지** — 기존 `data/processed/national_test_features.csv` 그대로 사용
- **출력 파일 5개 이외의 파일 수정/삭제 금지** — diagnosis_*.{json,md,png} 외에는 손대지 말 것
- 만약 경로 충돌(이미 같은 이름 파일 존재) 발생하면 덮어쓰지 말고 알려줄 것
